    ...
    def train_A3C_agent(self, max_episodes=Config.max_episodes, max_t=Config.max_t, stats_file=None):
        ...
        for episode_i in range(max_episodes):
            ...
            for t in range(max_t):  
                ...
                # select an action from the actor network by sampling from the action distribution output by the actor network
                ...
                # append the step info to the lists
            ...
            # compute the returns for each timestep in the episode    
            returns = self.compute_returns(final_value=0, normalize=False)    
                
                
            # compute the loss for each timestep in the episode    
            ...
            # take a gradient step on the critic network using the computed loss
            self.critic_optimizer.zero_grad()
            critic_loss.backward()
            self.critic_optimizer.step()
            
            # optimize the actor network using the computed loss by gradient step
...
if __name__ == "__main__":
    ...
    env = ContinuousCarRadarEnv()
    agent = A3C_agent(env, hidden_dim1=Config.hidden_dim1, hidden_dim2=Config.hidden_dim2, dropout=Config.dropout)
    agent.load_trained_network(Config.save_model_path + 'final_model/' + 'actor.pth', model='actor')
    agent.load_trained_network(Config.save_model_path + 'final_model/' + 'critic.pth', model='critic')
    agent.test_trained_agent(max_episodes=2, max_t=5000)
    pygame.quit()